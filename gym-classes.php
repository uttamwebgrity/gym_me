<?php 
include_once("includes/header.php");

$gym_small="class_photo/small/";
$gym_medium="class_photo/medium/";
$gym_original="class_photo/";



if(isset($_GET['action']) && $_GET['action']=='delete'){	
		
	$sql="select photo_name from classes_photos where class_id=" . (int) $_REQUEST['id'] . "";
	$result=$db->fetch_all_array($sql);
	
	if(count($result) > 0){
		for($i=0; $i<count($result); $i++){
			@unlink($gym_small.$result[$i]['photo_name']);	
			@unlink($gym_medium.$result[$i]['photo_name']);	
			@unlink($gym_original.$result[$i]['photo_name']);
		}
	}
	
	@mysql_query("delete from  classes_photos  where class_id=" . (int) $_REQUEST['id'] . "");
	
	$db->query_delete("classes","id='".$_REQUEST['id'] ."'");
	
	
	$_SESSION['user_message'] = "Your selected class deleted.";	   
	$general_func->header_redirect($general_func->site_url."gym-classes/");	
	
	
	
	/*$_SESSION['msg']="Your selected class deleted!";
	$general_func->header_redirect($_REQUEST['url']);*/
}
 




?>
<script language="JavaScript">


function del(id,name){
	var a=confirm("Are you sure, you want to delete class: '" + name +"'\nAnd all data related to it?")
    if (a){
    	location.href="<?=$_SERVER['PHP_SELF']?>?id="+id+"&action=delete";
    }  
} 
</script>			
<div class="main_container">
	<div class="main">
		<div class="text-content">
			<h1 style="float:left;">Classes</h1>
            <div class="add_new_product_to_product_list"><a href="class-new/"><span></span>Add new class</a></div>              
          	<?php          	
          	$sql="select id,name,description,status from classes where gym_id='" . $_SESSION['gym_id'] ."' order by name ASC";
			$result=$db->fetch_all_array($sql);
			$total=count($result);
			for($j=0; $j<$total; $j++){ ?>			
			<div class="edit_product_list_row">
							
				<div class="edit_product_detail full_width_list_detail">
					<h4><a><?=$result[$j]['name']?></a></h4>
					<div class="edit_del_product"><div><a href="class-new/<?=$result[$j]['id']?>"><img src="images/edit_product_icon.png" /><span>Edit</span></a></div><div><a onclick="del('<?=$result[$j]['id']?>','<?=$result[$j]['name']?>')"><img src="images/del_product_icon.png" /><span>Delete</span></a></div></div>
					<div class="div_clear"></div>
					<h5>Description</h5>
					<p><?=substr($result[$j]['description'],0,100)?>...</p>
					<div class="edit_del_product_info">
						<ul>
							<li style="margin-top:5px;"><strong>Status</strong>: <?=$result[$j]['status']==1?'Active':'Inactive';?></li>
                            <li><div class="add_edit_schedule"><a href="classes-schedule.php?class=<?=$result[$j]['name']?>&class_id=<?=$result[$j]['id']?>"><img src="images/schedule.png" />Add edit class schedule</a></div></li>
						</ul>
					</div>				
				</div>			
			</div>			
			<?php }	 ?>       
		</div>
	</div>
</div>
<?php include_once("includes/footer.php");?>