...
<div class="main_container">
	<div class="main">
		<div class="text-content">
            ...
            <div class="add_new_product_to_product_list"><a href="instructors-new/"><span></span>Add new instructor</a></div>
              
          	<?php          	
          	$sql="select id,name,description,photo_name,status  from instructors where gym_id='" . $_SESSION['gym_id'] ."' order by name ASC";
			$result=$db->fetch_all_array($sql);
			$total=count($result);
			for($j=0; $j<$total; $j++){ ?>			
			<div class="edit_product_list_row">
				<div class="edit_product_list_image">
					<?php                	
                	$img="instructor/small/".$result[$j]['photo_name'];
						
					if(!file_exists($img) || trim($result[$j]['photo_name']) == NULL)
				...
				<div class="edit_product_detail">
					...
					<div class="edit_del_product_info">
						<ul>
							<!--<li><strong>County</strong> : <?=$result[$j]['county']?></li>
							<li><strong>Area</strong>: <?=$result[$j]['area']?></li>-->
							<li><strong>Status</strong>: <?=$result[$j]['status']==1?'Active':'Inactive';?></li>
						</ul>
					</div>				
				</div>			
			</div>			
