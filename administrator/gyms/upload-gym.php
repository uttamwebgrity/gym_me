...
if(isset($_POST['enter']) && $_POST['enter']=="yes"){
	...
	for ($i = 1; $i <= $data->sheets[0]['numRows']; $i++) {					
		...
		if(trim($data->sheets[0]['cells'][$i][1]) != "Gym Name"){
			
		
			
			
			if(! $db->already_exist_inset("gyms","email_address",trim($data->sheets[0]['cells'][$i][2])) && ! $db->already_exist_inset("gyms","name",trim($data->sheets[0]['cells'][$i][1]))){
					
								
				$data_upload=array();							
				$data_upload['name']=trim($data->sheets[0]['cells'][$i][1]);
							
				$data_upload['seo_link']=$general_func->create_seo_link(trim($data->sheets[0]['cells'][$i][1]));
						
							
				if($db->already_exist_inset("gyms","seo_link",$data_upload['seo_link'])){//******* exit
					$data_upload['seo_link']=$db->max_id("gyms","id") + 1 ."-".$data_upload['seo_link'];
				}
							
				$data_upload['email_address']=trim($data->sheets[0]['cells'][$i][2]);		
				$data_upload['password']=$EncDec->encrypt_me(trim($data->sheets[0]['cells'][$i][3]));
				$data_upload['short_info']=trim($data->sheets[0]['cells'][$i][4]);		
				$data_upload['description']=trim($data->sheets[0]['cells'][$i][5]);
				$data_upload['youtube_videos']=trim($data->sheets[0]['cells'][$i][6]);
				$data_upload['website_URL']=trim($data->sheets[0]['cells'][$i][7]);
				$data_upload['phone']=trim($data->sheets[0]['cells'][$i][8]);
				$data_upload['street_address']=trim($data->sheets[0]['cells'][$i][9]);
				$data_upload['town']=trim($data->sheets[0]['cells'][$i][10]);
				$data_upload['area']=trim($data->sheets[0]['cells'][$i][11]);						
				$data_upload['county']=trim($data->sheets[0]['cells'][$i][12]);
				$data_upload['facebook_fan_page_link']=trim($data->sheets[0]['cells'][$i][13]);
				$data_upload['twitter_tweets_page_link']=trim($data->sheets[0]['cells'][$i][14]);
								
				$gen_lat=array();	
				$for_map=  trim($data_upload['area']) .",". trim($data_upload['area']).", " .trim($data_upload['town']) .", Dublin, Ireland";	
				$gen_lat=$general_func->getLnt($for_map);				
				$data_upload['geo_lat']=$gen_lat['lat'];	
				$data_upload['geo_long']=$gen_lat['lng'];
											
				$data_upload['email_confirmed']=1;
				$data_upload['membership_type']=2;
				$data_upload['status']=1;
				$data_upload['created']='now()';
				$data_upload['modified']='now()';			
				
				$db->query_insert("gyms",$data_upload);
  ...
  <tr>
    <td align="left" valign="top"><table border="0" align="left" cellpadding="0" cellspacing="0">
        <tr>
          ...
          <td align="left" valign="middle" class="body_tab-middilebg">Upload Gyms</td>
        ...
        <table width="883" border="0" align="left" cellpadding="0" cellspacing="0">
          ...
          <tr>
            ...
            <td width="780" align="left" valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="10">
                <tr>
                  <td  width="20%" class="body_content-form" valign="top">Upload Gyms (Excel file only):</td>
