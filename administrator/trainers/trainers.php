...
if(isset($_GET['action']) && $_GET['action']=='delete'){	
	...
	$sql="select logo_name from personal_trainers where id=" . (int) $_REQUEST['id'] . " limit 1";
	...
	$sql="select photo_name from personal_trainers where trainer_id=" . (int) $_REQUEST['id'] . "";
	$result=$db->fetch_all_array($sql);
	...
	@mysql_query("delete from  trainer_photos  where trainer_id=" . (int) $_REQUEST['id'] . "");
	$db->query_delete("trainer_outbound","trainer_id='".$_REQUEST['id'] ."'");
	
	$db->query_delete("personal_trainers","id='".$_REQUEST['id'] ."'");
...
function validate_search(){
	if(!validate_text(document.frmsearch.cd,1,"Enter trainer name.")){
		return false;
	}
}
  ...
  <tr>
    <td align="left" valign="top" class="body_whitebg"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                    ...
                    <tr>
                      <td align="center" valign="top"><table width="501" border="0" align="center" cellpadding="0" cellspacing="0">
                          <form name="frmsearch"  method="post" action="<?=$_SERVER['PHP_SELF']?>" onsubmit="return validate_search();">
                            ...
                            <tr>
                              <td width="123" align="right" valign="middle" class="content_employee" style="padding-right: 5px;">Trainer Name:</td>
        ...
        <?php
				...
				$sql="select id,name,email_address,email_confirmed,area,price_per_session,membership_type,membership_start,membership_end,status,last_login_date from personal_trainers ";				
...
cellpadding="5" cellspacing="1">
              <tr>
                <td  class="text_numbering">
                    ...
                    <tr>
                      <td width="5" align="left" valign="top"><img src="images/button-curve-left.png" alt="" width="5" height="22" /></td>
                      <td align="left" valign="middle" class="body_tab-middilebg"><input name="button" type="button" class="submit1" value="Add New" onClick="location.href='<?=$general_func->admin_url?>trainers/trainers-new.php'" /></td>
                      <td width="5" align="right" valign="top"><img src="images/button-curve-right.png" alt="" width="5" height="22" /></td>
                    </tr>
                  </table>
                  <?php }else echo "&nbsp;";?>
              ...
              <tr>
                ...
                <a href="trainers/trainers.php?display_oder=name&display_oder_type=<?=$display_oder_type?>" class="header_link">Trainer Name</a></td>
                ...
                <td width="100"  align="left" valign="middle" class="table_heading">Price Per Session</td>   
                <td width="50"  align="left" valign="middle" class="table_heading">Membership</td>  
                 <td width="70"  align="left" valign="middle" class="table_heading">
                 	
                 	 <a href="trainers/trainers.php?display_oder=status&display_oder_type=<?=$display_oder_type?>" class="header_link">Status</a></td>             
                <td width="50"  align="left" valign="middle" class="table_heading">Last Logged on</td> 
                
                <td width="80"  align="center" valign="middle" class="table_heading">Action</td>
              </tr>
              <?php if(count($result) == 0){?>
              <tr>
                <td colspan="8" align="center" bgcolor="#f5f7fa" valign="middle" height="50" class="message_error">Sorry, no trainers  found!</td>
              </tr>
              <?php }else{
              ...
              <tr bgcolor="<?=$j%2==0?$general_func->color2:$general_func->color1;?>">
                ...
                <td  align="left" valign="middle" class="table_content-blue">$<?=$result[$j]['price_per_session']?></td>
                  <td  align="left" valign="middle" class="table_content-blue"><?php                  
                ...
                <td  align="center" valign="middle" class="table_content-blue">
                	...
                	<img src="images/edit.png" onclick="location.href='<?=$general_func->admin_url?>trainers/trainers-new.php?id=<?=$result[$j]['id']?>&action=EDIT&return_url=<?=urlencode($url)?>'" style="cursor:pointer;"  title="EDIT" alt="EDIT" />
                	<?php } ?>
