...
if(isset($_GET['action']) && $_GET['action']=='delete'){
	...
	$db->query_delete("gym_services","service_id='".$_REQUEST['id'] ."'");
	$db->query_delete("client_services","service_id='".$_REQUEST['id'] ."'");
	
	$db->query_delete("job_services","service_id='".$_REQUEST['id'] ."'");
	
	$db->query_delete("services","id='".$_REQUEST['id'] ."'");
	$_SESSION['msg']="Your selected service deleted!";
...
function validate_search(){
	if(!validate_text(document.frmsearch.cd,1,"Enter service name")){
		return false
	}
}

function del(id,url,weapon){
	var a=confirm("Are you sure, you want to delete service: '" + weapon +"'?")
    if (a){
  ...
  <tr>
    <td align="left" valign="top"><table border="0" align="left" cellpadding="0" cellspacing="0">
        <tr>
          ...
          <td align="left" valign="middle" class="body_tab-middilebg">Services</td>
                        ...
                        <table width="543" border="0" align="center" cellpadding="0" cellspacing="0">
                          <form name="frmsearch"  method="post" action="<?=$_SERVER['PHP_SELF']?>" onsubmit="return validate_search();">
                           <input type="hidden" name="enter" value="3" />
                            ...
                            <tr>
                              <td width="115" align="right" valign="middle" class="content_employee" style="padding-right: 5px;">Service Name:</td>
         ...
         <tr>
          <td align="left" valign="middle" height="10"></td>
            ...
            <?php
				...
				$sql="select * from services $query order by $order_by";
...
cellpadding="5" cellspacing="1">
              <tr>
                <td  class="text_numbering">
                	...
                	<table border="0"  cellpadding="0" cellspacing="0">
                            <tr>
                              <td width="5" align="left" valign="top"><img src="images/button-curve-left.png" alt="" width="5" height="22" /></td>
                              <td align="left" valign="middle" class="body_tab-middilebg"><input name="button" type="button" class="submit1" value="Add New" onclick="location.href='<?=$general_func->admin_url?>settings/services-new.php'" /></td>
                              <td width="5" align="right" valign="top"><img src="images/button-curve-right.png" alt="" width="5" height="22" /></td>
                            </tr>
                   </table>
                   <?php }else echo "&nbsp;";?>
                   </td>
                <td class="text_numbering" align="right" colspan="2"><?=$total_count?> services found.</td>
              </tr>
              <tr>
               
                <td width="61%" align="left" valign="middle" bgcolor="#35619c" class="table_heading">
                <a href="settings/services.php?display_oder=name&display_oder_type=<?=$display_oder_type?>" class="header_link">Name</a>
                
                </td>
                <td width="19%" align="center" valign="middle" bgcolor="#35619c" class="table_heading">
                <a href="settings/services.php?display_oder=display_order&display_oder_type=<?=$display_oder_type?>" class="header_link">Display Order</a></td>
			...
			<?php if(count($result) == 0){?>
                	<tr>
                		<td colspan="3" align="center" bgcolor="#f5f7fa" valign="middle" height="50" class="message_error">Sorry, no services added yet!</td>
              		</tr>
				<?php }else{
					for($j=0;$j<count($result);$j++){?>
                     <tr bgcolor="<?=$j%2==0?$general_func->color2:$general_func->color1;?>">
                      ...
                      <?php if(isset($_SESSION['access_permission']['settings']['edit']) && intval($_SESSION['access_permission']['settings']['edit'])==1){ ?>
                       <img src="images/edit.png" onclick="location.href='<?=$general_func->admin_url?>settings/services-new.php?id=<?=$result[$j]['id']?>&action=EDIT&return_url=<?=urlencode($url)?>'" style="cursor:pointer;" />
