...
if(isset($_GET['action']) && $_GET['action']=='delete'){	
	...
	$db->query_delete("category","id='".$_REQUEST['id'] ."'");
	$_SESSION['msg']="Your selected class type deleted!";
...
function validate_search(){
	if(!validate_text(document.frmsearch.cd,1,"Enter class type")){
		return false
	}
}

function del(id,url,weapon){
	var a=confirm("Are you sure, you want to delete class type: '" + weapon +"'?")
    if (a){
  ...
  <tr>
    <td align="left" valign="top"><table border="0" align="left" cellpadding="0" cellspacing="0">
        <tr>
          ...
          <td align="left" valign="middle" class="body_tab-middilebg"> Class Types</td>
                        ...
                        <table width="543" border="0" align="center" cellpadding="0" cellspacing="0">
                          <form name="frmsearch"  method="post" action="<?=$_SERVER['PHP_SELF']?>" onsubmit="return validate_search();">
                           <input type="hidden" name="enter" value="3" />
                            ...
                            <tr>
                              <td width="115" align="right" valign="middle" class="content_employee" style="padding-right: 5px;">Class Type:</td>
          ...
          <td align="left" valign="middle" height="10"></td>
            ...
            <?php
				...
				if(isset($_REQUEST['display_oder']) && trim($_REQUEST['display_oder']) != NULL){
					...
					$order_by .="display_order, name ASC";
				}
				...
				$sql="select * from category $query order by $order_by";
...
cellpadding="5" cellspacing="1">
              <tr>
                <td  class="text_numbering">
                	...
                	<table border="0"  cellpadding="0" cellspacing="0">
                            <tr>
                              <td width="5" align="left" valign="top"><img src="images/button-curve-left.png" alt="" width="5" height="22" /></td>
                              <td align="left" valign="middle" class="body_tab-middilebg"><input name="button" type="button" class="submit1" value="Add New" onclick="location.href='<?=$general_func->admin_url?>settings/category-new.php'" /></td>
                              <td width="5" align="right" valign="top"><img src="images/button-curve-right.png" alt="" width="5" height="22" /></td>
                            </tr>
                   </table>
                   <?php }else echo "&nbsp;";?>
                   </td>
                <td class="text_numbering" align="right" colspan="3"><?=$total_count?> class types found.</td>
              </tr>
              <tr>
               
                <td width="35%" align="left" valign="middle" bgcolor="#35619c" class="table_heading">
                <a href="settings/category.php?display_oder=name&display_oder_type=<?=$display_oder_type?>" class="header_link">Class Type</a>
                
                </td>
                <td width="35%" align="left" valign="middle" bgcolor="#35619c" class="table_heading">Tag Slug</td>
                
                <td width="15%" align="center" valign="middle" bgcolor="#35619c" class="table_heading">
			...
			<?php if(count($result) == 0){?>
                	<tr>
                		<td colspan="4" align="center" bgcolor="#f5f7fa" valign="middle" height="50" class="message_error">Sorry, no class types added yet!</td>
              		</tr>
				<?php }else{
					for($j=0;$j<count($result);$j++){?>
                     <tr bgcolor="<?=$j%2==0?$general_func->color2:$general_func->color1;?>">
                        ...
                        <td  align="center" valign="middle" class="table_content-blue"> <input type="text" onclick="this.select();" name="sulg" value="<?=trim($result[$j]['seo_link'])?>"  class="form_inputbox" size="20" />  </td>
                      ...
                      <?php if(isset($_SESSION['access_permission']['settings']['edit']) && intval($_SESSION['access_permission']['settings']['edit'])==1){ ?>
                       <img src="images/edit.png" onclick="location.href='<?=$general_func->admin_url?>settings/category-new.php?id=<?=$result[$j]['id']?>&action=EDIT&return_url=<?=urlencode($url)?>'" style="cursor:pointer;" />
