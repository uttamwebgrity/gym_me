...
if(isset($_REQUEST['action']) && $_REQUEST['action']=="EDIT"){
	$sql="select * from specialty where id=" . (int) $_REQUEST['id'] . " limit 1";
	...
	$name="";
	$display_order=$db->max_id("specialty","display_order") + 1;
...
if(isset($_POST['enter']) && $_POST['enter']=="yes"){
	...
	if($_POST['submit']=="Add New"){
		if($db->already_exist_inset("specialty","name",$name)){
			$_SESSION['msg']="Sorry, your specified specialty is already taken!";
		}else{
			$data['name']=$name;			
			$data['seo_link']=$general_func->create_seo_link($name);
			
			//*** check whether this name alreay exit ******//
			if($db->already_exist_inset("specialty","seo_link",$data['seo_link'])){//******* exit
				$data['seo_link']=$db->max_id("specialty","id") + 1 ."-".$data['seo_link'];
			}
			//*********************************************//
			
			$data['display_order']=$display_order;
			$db->query_insert("specialty",$data);
			
			if($db->affected_rows > 0)
				$_SESSION['msg']="Specialty successfully added!";
				
			$general_func->header_redirect($_SERVER['PHP_SELF']);
		}	

	}else{
		if($db->already_exist_update("specialty","id",$_REQUEST['id'],"name",$name)){
			$_SESSION['msg']="Sorry, your specified specialty is already taken!";
		}else{
			$data['name']=$name;			
			$data['seo_link']=$general_func->create_seo_link($name);			
			//*** check whether this name alreay exit ******//
			if($db->already_exist_update("specialty","id",$_REQUEST['id'],"seo_link",$data['seo_link'])){//******* exit
			...
			$db->query_update("specialty",$data,"id='".$_REQUEST['id'] ."'");
			
			if($db->affected_rows > 0)
				$_SESSION['msg']="Specialty successfully updated!";
...
function validate(){
	if(!validate_text(document.ff.name,1,"Enter specialty name"))
		return false;
	if(!validate_text(document.ff.display_order,1,"Enter specialty display order"))
		return false;	
  ...
  <tr>
    <td align="left" valign="top"><table border="0" align="left" cellpadding="0" cellspacing="0">
        <tr>
          ...
          <td align="left" valign="middle" class="body_tab-middilebg"><?=$button?> Specialty</td>
        ...
        <table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
          ...
          <tr>
            <td align="left" valign="top" colspan="3"><table width="79%" border="0"  align="center" cellspacing="2" cellpadding="6">
                <tr>
                  <td width="15%" class="body_content-form" valign="top">Specialty Name:<font class="form_required-field"> *</font></td>
                  ...
                  <td width="85%"><table width="261" border="0" align="left" cellpadding="0" cellspacing="0">
                      <tr>
                        ...
                        <td width="49%"><table border="0" align="left" cellpadding="0" cellspacing="0">
                            <tr>
                              <td width="5" align="left" valign="top"><img src="images/button-curve-left.png" alt="" width="5" height="22" /></td>
                              <td align="left" valign="middle" class="body_tab-middilebg"><input name="back" onclick="location.href='<?=$general_func->admin_url?>settings/specialty.php'"  type="button" class="submit1" value="Back" /></td>
                              <td width="5" align="right" valign="top"><img src="images/button-curve-right.png" alt="" width="5" height="22" /></td>
                            </tr>
                          </table></td>
                      </tr>
